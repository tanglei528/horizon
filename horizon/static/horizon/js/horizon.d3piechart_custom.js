...
function create_label(chart) {
  return d3.select(chart).append("svg:text")
    .attr("dy", ".35em")
    .attr("text-anchor", "middle")
    .text(function(d) { return d[chart.attributes[1]["nodeValue"]] });
}
...
horizon.d3_pie_chart_usage = {
  ...
  pieChart: function(i) {
    ...
    var data = self.chart[0][i].attributes[1]["nodeValue"]
    ...
    var animate = function(data) {
      vis.selectAll(".arc")
        ...
        .attr("d", arc)
        .style("fill", function(){
          if (self.data[0].percentage >= 80) {
            return FULL;
          } else if (self.data[0].percentage >= 50) {
            return NEARLY_FULL;
          } else {
            return FRGRND;
          }
      ...
      vis.append("svg:text")
        .attr("dy", ".35em")
        .attr("text-anchor", "middle")
        .text(function(d) { return data[0]["percentage"]+"%"; });
    };
